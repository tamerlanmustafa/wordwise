...
- **The one exception is the open film (`movieDetail`), which hides the bar** (`GlobalBottomBar hidden`, since 2026-09-14): on an iPhone SE the capsule's height left the deck's card at 70%. That screen pads by `deckBottomClearance(insets.bottom)` instead — don't add a bar inset back to it. Short phones there also get a tighter header (`compactColumnFor` in `vocabulary/deckMetrics`).
