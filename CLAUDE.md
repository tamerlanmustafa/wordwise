...
## Every screen clears the bottom bar — and the status bar
- **`GlobalBottomBar` is an absolute overlay drawn on top of every screen, including deep screens (Settings, Account, the paywall).** It takes no space from its siblings, so nothing moves out of its way unless the screen makes room. Anything that doesn't is drawn *behind a translucent capsule*: visible, and not tappable. Consider it for every new page and every layout change, not just new scrollers.
- **No screen hides the bar.** The film screen hid it on 2026-09-14 to give its word card height on an iPhone SE, and it was put back by request the same day. When a short phone runs out of height the screen gives way instead: the film screen tightens its header and deck gaps, then switches to a compact card (`deckLayoutFor` in `vocabulary/deckMetrics`).
- **A scroller reserves the bar's height:** `paddingBottom: useBottomBarInset() + 24` on its `contentContainerStyle`. Never a hard-coded number (`120`, `56 + 24`) — it doesn't track the safe area or the floating/pinned split.
- **That only makes the *last* element reachable. It says nothing about what sits under the bar at rest.** The paywall followed the rule exactly and still rested its "Get Plus" button behind the capsule. So also ask: *with the screen at rest on a small phone, what is under the bar?* If it is the screen's primary action, pin it.
- **Pin a bottom action as a sibling *below* the `ScrollView`, padded by the inset** — `<View style={[s.footer, { paddingBottom: barInset }]}>` with an opaque background — not as an absolute overlay. Layout then places it above the bar and the scroll shrinks to fit; nothing is measured. When the footer is present, the scroll above it no longer reserves the bar (see `PaywallScreen`).
- **Sheets are the exception:** they cover the full screen and pad *inside* by the bar's height, because the bar draws over them anyway (`common/BottomSheet`).
- **The top edge: use `TopInsetView`, never `<SafeAreaView edges={['top']}>`.** `SafeAreaView` applies its padding during native layout, a frame after the content paints, so a screen's first frame is drawn under the status bar and then jumps down. Deep screens remount *every* time they open, so this was a visible glitch on every visit — measured as one frame drawn 62pt too high when swiping back to Profile. `TopInsetView` reads `useSafeAreaInsets()` and pads in the same commit.
- Guarded by `components/__tests__/bottomBarClearance.test.ts` and `components/common/__tests__/topInsetView.test.ts`. Check both, and look at the screen on the simulator with the bar visible, before calling UI work done.

## Every layout works on a short screen
- **The smallest phone we support is the iPhone SE (375×667pt: 20pt status bar, no home indicator).** Many Android phones are as short (~640–720dp), and 3-button navigation makes the pinned bar taller still. The iPhone 17 Pro the simulator defaults to is 874pt — **207pt of room a small phone does not have.** A layout checked only there is unchecked.
- **The failure: fixed-height blocks stack up, and whatever is left over goes to the part that matters.** A screen's top inset + a fixed header + the bar's reserve are the same on every phone; only the space between them shrinks. On the SE that leaves ~547pt for the Practice path against 712pt on the 17 Pro.
- **Don't place the thing the user is looking for by counting from one edge.** Practice opened scrolled to its bottom with four completed tiles under START, so the road ahead got whatever height was left: two tiles on the 17 Pro, *none* on the SE, with START pressed 8pt under the streak panel. Place it relative to the *visible* area instead — Practice now centres START between the panel and the bar (`practice/pathCentering`) — and let the buffer on either side be the part that shrinks.
- **"Visible" excludes the bar.** The middle of a scroller's box is half a bar lower than the middle of what is on screen (see "Every screen clears the bottom bar").
- **Verify on a short simulator before calling UI work done.** Create one if it is missing — `xcrun simctl create "WW iPhone SE 3" com.apple.CoreSimulator.SimDeviceType.iPhone-SE-3rd-generation <iOS runtime>` — install the same `.app` the main simulator runs (`xcrun simctl get_app_container <main UDID> com.wordwise.mobile app`), open it through the dev-client link, and compare it side by side with the tall phone. idb taps near the bottom of an iPhone 13 mini land ~30pt low (its screen scale is 2.88, idb assumes 3) — prefer the SE.
