...
- [x] **[You]** Set up email at your domain: Cloudflare → **Compute → Email Service → Email Routing** (free; it moved out of the `Email` tab, which now holds DMARC Management + Email Security) → forward `support@getwordwise.us` + `privacy@getwordwise.us` → a dedicated WordWise Gmail. **Done 2026-08-28** (#159). Apex now has three `route*.mx.cloudflare.net` MX records, `v=spf1 include:_spf.mx.cloudflare.net ~all`, and a `cf2024-1._domainkey` DKIM — a *different selector* from Resend's, so `resend._domainkey` and the `send.getwordwise.us` SPF + feedback MX are untouched and outbound is unaffected. Both addresses receive real mail. ⚠️ Forwarded mail lands in **Gmail spam** until the destination account builds reputation — a "never send to spam" filter on both addresses is the fix, and it matters because a GDPR deletion request in spam is the failure this checkbox exists to prevent.
