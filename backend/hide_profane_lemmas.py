"""
Hide swear words and slurs that are already stored in prod vocabulary.

src/services/profanity_filter.py stops new profanity at classification time
(via the lemma guard), but everything ingested before that guard existed is
...
word by removing its `hidden_words` row. (The destructive counterpart,
purge_impure_lemmas.py, now also deletes these rows when run with --apply,
because it shares the same lemma guard.)

Run from backend/:
    python3.11 hide_profane_lemmas.py              # dry run (default)
    python3.11 hide_profane_lemmas.py --sample 40  # spot-check what matched
    python3.11 hide_profane_lemmas.py --apply      # actually hide
"""
...
from src.services.profanity_filter import is_profane, is_profane_entry

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(name)s: %(message)s",
)
logger = logging.getLogger("hide_profane_lemmas")
...
async def fetch_candidates(db: Prisma) -> List[Tuple[str, str]]:
    ...
    for row in wc_rows:
        ...
        if not word:
            continue
        if not is_profane_entry(word, lemma):
            continue
...
async def main():
    ...
    try:
        ...
        logger.info(
            f"Profane forms found: {len(candidates)} "
        ...
        for i in range(0, len(to_hide), BATCH):
            ...
            for form, _src in chunk:
                params.extend([form, admin_id, "auto: profanity"])
            sql = (
