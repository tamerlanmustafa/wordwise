...
from ..services.streak_service import grant_weekly_if_due, max_held_for
...
async def complete_session(
    ...
    if total_count > 0 and credited and first_completion:
        ...
        # The cap is the account's, not the app's: a free user banks two, a
        # Plus user five. The RATE is deliberately identical — the grant is
        # earned by practising, and tiering the habit is the one thing this
        # design does not do.
        await grant_weekly_if_due(
            db,
            user_id=current_user.id,
            today=today,
            max_held=max_held_for(current_user),
            now=now,
        )
    ...
    reward = await award_session_chest(
        db, user_id=current_user.id, max_held=max_held_for(current_user)
    )
    await db.user.update(
        where={"id": current_user.id},
