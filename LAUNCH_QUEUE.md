...
| #162 #163 | **Both built. Install the #163 preview build (runtime `1.0.3`, IDs on the issue), not the cd1a9ec one** — the `1.0.2` builds (iOS `ee14cc22` / Android `06f8d8b4`) no longer receive OTAs. Then the device walk: iOS + Android, free + premium, Reduce Motion on, silent switch on — and a premium **pronunciation tap on both platforms**, which had never worked (the endpoint is bearer-gated and the old code sent no header). Watch the first App Store upload for ITMS-90683 (`NSMicrophoneUsageDescription`) — `expo-audio` links recording APIs; detail on the issues. (Items 43, 44) |
