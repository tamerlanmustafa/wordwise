...
| #162 | **Feedback module shipped 2026-09-01, uncommitted, needs an `eas build`** (new native deps — an OTA carrying them crashes on launch). Batch with #163, which needs the same rebuild. Then the device walk: iOS + Android, free + premium, Reduce Motion on, silent switch on. Watch the first App Store upload for ITMS-90683 (`NSMicrophoneUsageDescription`) — both `expo-av` and `expo-audio` link recording APIs; detail on the issue. (Item 43) |
