...
| 2026-09-01 | 45 (#164) | Shipped screeningLogic.ts (partitionScenes, beatsForScene, pickTestWords, requeue, linear cursor with a real `exhausted`) + screeningStore.ts (per-film AsyncStorage progress; the 24h rule restarts only the in-flight scene). 6-card scene = 3 / spot check / 3 / test of 3+2 = 12 beats. deckLogic untouched. |
