...
| 2026-09-02 | 48 (#165) | Shipped ScreeningScene + screeningRunner: a film opens into its lesson (cards → spot check → cards → test), wrong answers requeue, every beat and score is written as it happens so a quit resumes exactly. WordCardDeck gained one opt-in `onExhausted`. Two follow-ups on #165; device walk unrun. |
