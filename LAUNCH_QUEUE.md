...
| 46 | `done` | #166 | `movie_lesson` session kind: composer + **lazy** `UserWord` creation for tested words only (precedent: `compose_list_words`). Every answer posts to the existing `/srs/review` — do not build a second memory model. Item 42 decided: energy **replaces** the daily cap, so `movie_lesson` is not subject to `srsLastSessionStartedAt`. |
| 47 | `done` | #167 | Distractor pool falls back to deck-only on a cold language; in an 8-word scene that is a shell game by question three. Add a film-wide rung between the wide pool and the deck. **Check the prod pool-size log line first** — this may be CLOSE rather than RESOLVE. |
| 48 | `done` | #165 | The scene runner: 4 cards → spot check → 4 cards → 6-question test, wrong answers requeued until right. Reuses `MCQCard`, `QuizHeader`, `SessionComplete`, `Confetti`. **A scene must never be lost** — running out pauses resumably. Screening Mode is the **default** on MovieDetailScreen (item 42). Needs 45, 46, 47. |
...
| *(no issue)* | **Walk Screening Mode on a device — #165 shipped 2026-09-02 and closed, but nothing has driven a scene on a phone.** The dev `.env` TMDB key is revoked (401 → proxy 502), so the simulator cannot reach any of the six locally-scripted films; the sim build boots on the commit and MovieDetailScreen renders with the lesson correctly closed for a deckless film, and that is all that was proven. On the preview build (runtime **1.0.3** + OTA `0ff92b29`): open a film → it should open into the lesson, cards → spot check → cards → test → complete; miss a question and check it comes back; quit mid-scene and reopen; swipe a card "I know this" and check it leaves that scene's test; "Browse the deck" and back in via the header chip. iOS **and** Android, free **and** premium. (Item 48) |
...
| 2026-09-02 | 47 (#167) | Film rung between the wide pool and the deck: on a cold language a scene test draws wrong answers from the film's own translated words (cache + gloss, one 2ms read, no API spend), gated on `is_thin`. Prod caches 18,317 lemmas for TR vs 48 ES. POS-restricted after prod returned `that`/`with`. |
| 2026-09-01 | 46 (#166) | Shipped `movie_lesson`: `?kind=movie_lesson&movie_id=N&words=…` returns one card per word; UserWord rows created lazily for tested words only, existing rows reused, learned words skipped. Exempt from the free daily cap and never stamps it. No film-membership check — prod tables agree on only 96%. |
| 2026-09-02 | 48 (#165) | Shipped ScreeningScene + screeningRunner: a film now opens into its lesson (cards → spot check → cards → test), wrong answers requeue, every beat and score is written as it happens so a quit resumes exactly. WordCardDeck gained one opt-in `onExhausted`. Follow-up on #165: `missed` monopolises both resurface slots after scene 2. |
