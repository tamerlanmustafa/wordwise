...
// ---------------------------------------------------------------------------
// 5. No screen hides the bar
// ---------------------------------------------------------------------------

/**
 * The film screen hid it for an afternoon (2026-09-14) to give its word card
 * height on an iPhone SE, and it was put back by request: the bar is on every
 * screen. A short phone gives up header, gaps and card slots instead — see
 * `vocabulary/deckMetrics.deckLayoutFor`.
 */
describe('no screen hides the bar', () => {
  it('renders the bar unconditionally, with no way to hide it', () => {
    const app = code(path.join('core', 'App.tsx'));
    expect(app).toMatch(/<GlobalBottomBar active=\{activeTab\}/);
    expect(app).not.toMatch(/<GlobalBottomBar[^>]*hidden/);
    expect(code(path.join('components', 'GlobalBottomBar.tsx'))).not.toMatch(/display:\s*'none'/);
  });

  it('the film screen reserves the bar like every other screen', () => {
    const film = code(path.join('components', 'screens', 'MovieDetailScreen.tsx'));
    expect(film).toMatch(/paddingBottom:\s*barInset/);
    expect(film).toMatch(/bottomOffset=\{barInset\}/);
  });
});
